...
_TYPE = {
    "paper": ("Paper", "ic"), "talk": ("Talk", "sla"), "award": ("Award", "du"),
    "group": ("Group", "ii"), "software": ("Software", "violet"),
}
...
def _news_card(n):
    ...
    year = n.get("year", 0)
    link = n.get("link", "")
    more = f' <a class="reslink" href="{link}" target="_blank" rel="noopener">Read more →</a>' if link else ""
    search = attr_esc(f'{n.get("title","")} {n.get("blurb","")} {label}')
    return (
        ...
        f'<div class="item-tags"><span class="badge">{esc(label)}</span></div>'
        f'</article>'
    )
