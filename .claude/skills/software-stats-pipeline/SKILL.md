...
- **`repos{}` is owned by the fetch script.** Every write sets `lastUpdated` **and** `lastFetched` to the run date and stores a `reposChecksum` over the repos map; the write also refuses an empty `repos{}` and refuses any repo missing the `archived` flag the page renders. If you hand-add a repo (a collaborator ships one mid-cycle and re-running the fetch isn't practical), you **must** bump `lastUpdated` yourself and append to the `manualEdits` list — otherwise the file advertises a timestamp older than the data it contains. The next real fetch detects the checksum mismatch, prints which cache was hand-edited, carries `manualEdits` forward as a log, and replaces the stats.
- **Cache fields**: `lastUpdated` (date the contents last changed) · `lastFetched` (date of the last full GitHub/PyPI pull) · `githubUser` · `manualEdits[]` (`{date, summary}`) · `reposChecksum` · `repos{}`. Per repo: `url`, `stars`, `forks`, `language`, `description`, `pushed`, `archived`, `isFork`, `downloads_month`, and `external` for collaborator repos. `archived` is rendered by the page, so it is required on every repo.
