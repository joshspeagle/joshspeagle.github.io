...
The Mentorship page uses the shared grouped listview (`assets/js/redesign/listview.js`): a search box plus two chip rows (career stage, current/former) filter `[data-lv-item]` cards within `[data-lv-group]`/`[data-lv-section]`, collapse empty groups and sections, and update their `[data-lv-count]`/`[data-lv-seccount]` tallies. The overview tiles and the career-stage chart sit outside that root and stay all-time totals (captioned as such).
