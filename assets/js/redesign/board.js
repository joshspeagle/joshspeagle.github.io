/* board.js — the page is a board. Measures every [data-chip] after layout, routes copper
   traces from a trunk in the left gutter into each chip's pin, and sends a pulse of energy
   along the trace when the chip scrolls into view.

   The trunk is fed from the hero network on Home and from the header band's bottom-left on
   every other page; when a page has neither, it simply starts at the top of <main>. Static
   traces are pure SVG; motion is gated by prefers-reduced-motion (a parked pulse + lit pins
   instead). No JS = no board — #board stays empty and nothing else changes.

   Density: keep it to <= 8 traced [data-chip] blocks per page. List cards are chips
   visually but carry no trace; one branch feeds the whole list column. */
(function () {
  ...
  let raf = null, idle = null, trunkPath = null;
  ...
  const abs = (r) => ({ left: r.left + scrollX, top: r.top + scrollY, right: r.right + scrollX,
  ...
  function build() {
    ...
    // The content column — the same .container every section uses, so the trunk sits in
    // the page's real left gutter rather than beside some narrower inner block.
    const cont = (main && main.querySelector('.container')) || document.querySelector('.container');
    const hero = document.querySelector('.hero');
    const band = document.querySelector('.pub-header');   // secondary pages' header band
    ...
    // 1. Feed. On Home the hero network's output leaves the canvas, runs along the hero's
    //    bottom edge and drops into the trunk (45 deg corners only); elsewhere the feed
    //    leaves the header band at its bottom-left.
    let trunkTop;
    if (hero) {
      const h = abs(hero.getBoundingClientRect());
      const outX = narrow ? W * 0.5 : W * 0.69, outY = h.bottom - 70;
      trunkTop = h.bottom + 6;
      el('path', { d: `M${outX} ${outY} V${h.bottom - 30} L${outX - 16} ${h.bottom - 14} H${tx + 16} L${tx} ${trunkTop + 2}`,
                   class: 'tr tr-feed' }, g);
      el('use', { href: '#via', x: outX - 5, y: outY - 5, width: 10, height: 10, class: 'cu' }, g);
    } else if (band) {
      // Secondary pages: the feed leaves the header band at its bottom-left. It stays
      // inside the band's bottom padding so it can never cross the tagline, however
      // far that wraps.
      const b = abs(band.getBoundingClientRect());
      const outX = Math.max(tx + 56, Math.min(c.right - 40, c.left + 340));
      trunkTop = b.bottom + 6;
      el('path', { d: `M${outX} ${b.bottom - 30} L${outX - 16} ${b.bottom - 14} H${tx + 16} L${tx} ${trunkTop + 2}`,
                   class: 'tr tr-feed' }, g);
      el('use', { href: '#via', x: outX - 5, y: b.bottom - 35, width: 10, height: 10, class: 'cu' }, g);
    } else {
      trunkTop = abs(main.getBoundingClientRect()).top + 24;
    }

    // 2. Trunk down the gutter to the footer ground.
    const trunkBottom = foot ? abs(foot.getBoundingClientRect()).top + 28 : H - 60;
    if (trunkBottom <= trunkTop + 40) return;
    const trunk = el('path', { d: `M${tx} ${trunkTop} V${trunkBottom}`, class: 'tr tr-trunk' }, g);
    trunkPath = trunk;
    idle = { path: trunk, len: trunk.getTotalLength(),
             glow: el('circle', { r: 7, class: 'idle-glow', opacity: 0 }, g),
             dot: el('circle', { r: 1.8, class: 'idle', opacity: 0 }, g), t: 0.35 };
    ...
    chips.forEach(chip => {
      ...
      if (!r.width || r.top < trunkTop) return;              // laid out above the trunk: skip
      const busKey = chip.closest('[data-bus]');
      let d, pinX, pinY, pinSide;
      if (busKey && !narrow) {
        // bus: a horizontal trace 18px above the row, tapped from the trunk once; each
        // chip in the row drops into it from its top edge
        ...
        d = narrow ? `M${tx} ${pinY - 14} L${tx + 14} ${pinY} H${pinX - 6}`
                   : `M${tx} ${pinY - 16} L${tx + 16} ${pinY} H${pinX - 6}`;
    ...
    if (idle) {                        // a slow pulse rides the trunk continuously (parked when still)
      const place = (t) => {
        const p = idle.path.getPointAtLength(idle.len * t);
        idle.dot.setAttribute('cx', p.x); idle.dot.setAttribute('cy', p.y);
        idle.glow.setAttribute('cx', p.x); idle.glow.setAttribute('cy', p.y);
        idle.dot.setAttribute('opacity', 1); idle.glow.setAttribute('opacity', 0.7);
      };
      place(idle.t);
      if (!still) {
        const t0 = performance.now(), period = 9000;
        (function ride(now) {
          if (trunkPath !== trunk) return;                    // a rebuild replaced this trunk
          idle.t = ((now - t0) % period) / period;
          place(idle.t);
          requestAnimationFrame(ride);
        })(t0);
      }
    }
    if (still) {                       // reduced motion / render: park a pulse mid-trace, light some pins
      routes.forEach((r, i) => {
        if (i % 4 === 1) {                       // one chip in four shows the pulse arriving
          ...
          r.glow.setAttribute('cx', p.x); r.glow.setAttribute('cy', p.y); r.glow.setAttribute('opacity', 0.6);
        }
  ...
  function tick(now) {
    ...
    for (const r of Array.from(active)) {
      ...
      if (t >= 1) {
        active.delete(r); r.lit = true;
        r.pulse.setAttribute('opacity', 0); r.glow.setAttribute('opacity', 0);
        r.pin.classList.add('lit'); r.chip.classList.add('lit');
        setTimeout(() => r.chip.classList.remove('lit'), 1800);
      }
    }
  ...
  function observe() {
    ...
    io = new IntersectionObserver(es => es.forEach(e => {
      ...
      const r = routes.find(x => x.chip === e.target);
      if (r) fire(r);
    }), { threshold: 0.35 });
